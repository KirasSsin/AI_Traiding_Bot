...
# paths. Same guard pattern as adr-*-sync-check.sh.
case "$command_str" in
    *"adr-agent-sync-check.sh"*|*"adr-index-sync-check.sh"*|*"wiki-broken-link-check.sh"*) exit 0 ;;
    *"hooks/"*"sync-check"*|*"hooks/"*"-check.sh"*) exit 0 ;;
esac
