...
description: Read-only SHIP orchestrator для AI Trading Bot v0.1 kit — verifies sprint-NN wiki page completeness, changelog derived from main..HEAD commits, tag-version sequence continuity, skill-manifest 7/7, SPRINT_STATE budget+phase, и proposes (never executes) the squash-commit message + git tag/push/merge command sequence. MUST BE USED before any `gh pr merge` OR `git tag` invocation in PHASE 8 Ship. Use proactively when user says "ship", "финишируем", "готовим релиз", "tag", "release checklist", OR when sprint-finish skill reaches Step 7 (Commit + ship). Complements sprint-finish (which performs actions) by giving a read-only pre-flight verdict — analogous to how architecture-reviewer complements refactor authors. NOT for architectural review (architecture-reviewer), NOT for security/money-path review (security-auditor), NOT for actually running git tag/merge/push (sprint-finish + finishing-a-development-branch skill do that), NOT for docs content quality (doc-reviewer).
