...
- **Сценарий:** 24 задачи 6 блоков (Bucket 0: subdirs refactor / Bucket A: backend / Bucket B: frontend bugs / Bucket C: Bybit balance / Bucket D: Glossary tab / Bucket E: RU язык + wiki sync)
- **Frontend:** Component subdirs refactor (tabs/charts/forms/metrics/shared/glossary) per architect C5; EquityChart 3-line tooltip + initialBalance; MetricsTable visual divider GATE-BLOCKING vs INFORMATIONAL; FailAnalysisTab упрощение к chips + Glossary links; HistoryTab accordion expand с RU summary (single-open + ESC); GlossaryTab NEW (section TOC + per-strategy filter + search + anchor deeplinks); BalanceBadge (LIVE/CACHED/OFFLINE/Loading states); useStrategyContext URL query state hook (architect C2)
- **Backend:** account_service.py wrapper per architect C1; /api/bybit/balance + /api/glossary endpoints; replay engine equity_curve emission (Bug B fix); glossary_data.py ~40 RU entries + STRATEGY_TO_METRICS_MAP; RunRecord initial/final balance + win_rate fields; initial_balance threading через BacktestPayload + run_backtest()
- **Architect bindings met:** C1 (account_service wrapper) / C2 (URL query state useStrategyContext) / C3 (single glossary endpoint) / C4 (HistoryTab accordion single-open) / C5 (component subdirs)
- **Tests:** Vitest 32 (+9 vs S47: useStrategyContext 4 + HistoryTab 3 + MetricsTable 2) / Playwright 7 (+3: equity-chart-all-presets research/WFA/empty) / pytest 1056+ (+account_service + balance endpoint + glossary + RunRecord + replay equity)
- **Operator UI bugs:** A tooltip 3-line + balance USDT / B chart все стратегии (replay equity_curve) / C Bybit balance LIVE fetch / D informational distinction opacity+divider / E Glossary tab CORE (4-й tab) / F chips упрощение / G triangle ▸ remove / H accordion expand + RU summary / I language enforcement CLAUDE.md table
...
## [2026-05-29] ship | S50 v0.1.0-alpha.50 — Supertrend WFA_FAIL + reusable infra

- Squash-merge `81d4bc7` (PR #61) → tag `v0.1.0-alpha.50`
- Hypothesis #10 Supertrend (freqtrade Lazybear adaptation) — verdict WFA_FAIL (honest 10th conjoint fail). Strategy NOT promoted; dashboard comparison-only.
- Primary failure: T5 n_eff<50 (gate-blocking, DSR-independent). DSR=0.0 = uninformative/artifactual (contaminated cross-trial pool).
- PHASE 6 BLOCKER (trading-logic): backtest same-bar fill look-ahead (open[i] vs open[i+1], ~+117% inflation, 3 paths) — FIXED + 2 TDD guards. Held-out 8.08→−4.23 (honest). Streaming clean (untouched), 4-way parity preserved.
- Reusable infra: wilder_atr() shared (indicators.py), autoresearch held-out split (anti-champion-bias, operator Q4), supertrend_runner, SupertrendStrategy streaming.
- Post-review fixes: DSR-uninformative doc, cross_trial tag fallback (multiplier vs atr_breakout_mult), strengthened n_trials wiring test.
...
## [2026-06-26] sprint-end | S55 — Full-project audit + refactor

- **Аудит → refactor (не feature-спринт).** Workflow `w1hxvgkoa` на shipped main (tag alpha.54): 120 агентов / 9 измерений / 2× скептик-верификация → **43 подтверждённых дефекта** + bonus QS-3. Все исправлены TDD + 2 раунда re-review (PHASE 6 + 6.2), которые выявили ещё **7 follow-up-фиксов**.
- **2 BLOCKER:** TL-01 (live-runtime никогда не вооружал OCO + ронял exit-сигналы → unbounded-loss; entry-fill не триггерил arm_oco, exit-сигналы не доходили до flatten) + BYBIT-01 (REST/WS разные Bybit env → fill-loop сломан).
- **9 HIGH:** TL-02, BYBIT-02 emergency-flatten tri-state, BYBIT-03 unified error hierarchy, ARCH-02 reconcile I/O off-lock, QS-1 DSR de-annualization, DI-01 multi-gap bars, DI-02 BarSource unclosed-bar drop, SEC-S55-01 path-traversal allowlist, DASH-01 RAW_PRETRAIN render.
- **15 MEDIUM:** TL-03/04 streaming exit-priority, ARCH-03 public adapter API, BYBIT-04 residual orderLinkId, BYBIT-05 residual step-floor dust, DI-03 migration integer-sort, DI-04 Bar tz-aware, DASH-02 MonthlyHeatmap true monthly return, DASH-03 atomic cache write + single-flight lock, QS-2 `_cmd_wfa` DSR units (ADR 0071), TQ-01..06.
- **17 LOW:** ARCH-05 layering → `src/backtest/data_loading.py`, TL-06 KronosStrategy `_current_side`, TL-07 ReasonCode enums, QS-2-bars 4H 2191, DI-06/SEC-S55-03/PY-5 atomic `prediction_cache.put`, SEC-S55-04 WS log allowlist, PY-1..4 CI/typing/DRY, TQ-07/08, DASH-04 HistoryTab RAW, DASH-05 OPTGROUP_ORDER, BYBIT-06 kline stall guard. + bonus QS-3 donchian DSR twin.
- **PHASE 6/6.2 follow-up (7):** SEC-BYBIT01-INCOMPLETE (HIGH — `demo=` на всех 4 RESTClient + AST-gate), ARCH-02-REG-01 (HIGH — bootstrap RLock поверх REST I/O), ARCH-03-REGRESSION (HIGH — integration `_FakeAdapter` props; integration-тесты не были в default-gate), TL-NEW-01 (MEDIUM — 2 новых FSM-перехода), DASH-03-GAP-01 (MEDIUM — Kronos atomic writes через `src/dashboard/_cache_io.py`), QS-6 (MEDIUM — `__main__` 4H bars_per_year→2191), NEW-LOW-01 (DRY — консолидация `atomic_write_text`).
- **Canonical counts:** FSM transitions **74→76** (TL-NEW-01: (LONG_OPEN, FLATTEN_FAILED)→HALTED + (OCO_ARMING, FLATTEN_FAILED)→HALTED). States=16, events=30, reason_codes=67 — без изменений. Verify: `16 30 76 67`.
- **Новые модули:** `src/backtest/data_loading.py` (ARCH-05), `src/dashboard/_cache_io.py` (DASH-03-GAP-01), `src/ml/weights_hash.py` (TQ-05). Миграция: `migrations/0007_bracket_exit_prices.sql` (TL-01).
- **CARRY S56: BYBIT-08** — `coordinator._try_place_market_sell` bare-except классифицирует post-`retCode==0` OrderAck-parse-failure как NOT_SENT → flatten attempt-2 → double-sell. Pre-existing + редкий. Корректный фикс = adapter-level typed `AmbiguousOrderOutcome` через 3 `place_*`-варианта. Требует своего ADR/спринта.
...
## [2026-07-02] ship | S62 — Manifest & Telemetry (mega-run 6/8)
- «Скилл выстрелил / гейт сработал» из надежды → проверяемый артефакт: skill-manifest.sh (7 фаз, STOP до тега; догфуд 6/6 ✓), cascade-read-check (WARN на full-read banned/>50КБ), tamper-evidence review-gate (review-sNN закоммичен в диапазоне мерджа + строка ревьюера — закрыл эфемерность S59/S61). ADR 0074 tuning (A/B-методика). KIT-022 (block-list source_files → WARN). KIT-025 аудит (13 хуков safe python3 -c; docs-staleness 2 heredoc без backtick).
- Ревью: architecture APPROVE_WITH_CONDITIONS (Phase-7 hard-STOP→advisory, Phase-4 regex-якорь — оба класса прошлых находок) + security **HIGH #1** (origin-strip auth-bypass money-гейта, живой с S59: `git merge origin/feature/sprint-NN` минул детект → silent exit 0) ЗАКРЫТ (валидация merge_ref + origin/ retry). MEDIUM #2 (T2 не привязан к money-диффу) → backlog KIT-OD-2.
- Тег v0.1.0-alpha.62 локально. → S63 Fable-5 Team.
...
## [2026-07-02] ship | S65 — Error-Harvest & Kit Hardening (mega-run)
