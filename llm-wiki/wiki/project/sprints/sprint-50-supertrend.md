...
| **T8** sweep + held-out eval | — | Autoresearch sweep на train (< 2025-06-01). Winner: atr=21/mult=2.0 (held-out Sharpe=8.08 → PROCEED). **PHASE 6 fix `48dac93`:** held-out 8.08 был look-ahead-inflated; после fill-fix winner=atr=7/mult=3.0, held-out Sharpe=−4.23, PnL −42.5% → T8 verdict FAIL. | DONE — изначально PROCEED, после fix FAIL |
...
**Числа после fix (T8 re-run, `scripts/run_supertrend_s50.py`):** winner сменился atr=21/mult=2.0 → **atr=7/mult=3.0** (pick_winner = highest train Sharpe среди eligible). Train Sharpe 8.70→**0.27** (макс по всей сетке упал с 8.7 до 0.27). Held-out Sharpe **8.08 → −4.23** (сигнал отрицательный OOS), held-out PnL **+152.8% → −42.48%**, n_trades 162→98. Большинство комбо теперь с отрицательным PnL. T8 verdict стал FAIL (был PROCEED). **Финальный T9 verdict остаётся WFA_FAIL** — исправление делает fail ЖЁСТЧЕ (OOS n_eff 47→16), подтверждая прогноз reviewer: убирание look-ahead уменьшает fillable-сделки.
