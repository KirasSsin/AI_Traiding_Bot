...
|---|---|---|---|
| 1 | INIT | S5 | Начало процесса, до загрузки состояния |
| 2 | FLAT | S5 | Нет открытых позиций |
| 3 | ENTRY_PENDING | S5 | Entry-ордер отправлен, ожидание fill |
| 4 | LONG_OPEN | S5 | Позиция открыта, OCO ещё не выставлен |
| 5 | OCO_ARMING | **S6** | Entry заполнен, размещение TP/SL в процессе |
| 6 | OCO_ARMED | S5 | Позиция + OCO активен (нормальный режим) |
| 7 | PARTIAL_FILL | S5 | Legacy S5 — в v2 недостижим (событие PARTIAL_FILL → EXIT_SL_RESIDUAL); сохранён для back-compat при загрузке SQLite |
...
| 11 | EXIT_SL_RESIDUAL | **S6** | IOC частичный SL fill, выравниваем остаток |
| 12 | RECONCILING | S5 | Post-reconnect сверка |
| 13 | HALTED | S5 | Risk halt — держим позицию, не торгуем |
| 14 | COOLDOWN | S5 | Пауза перед re-entry |
| 15 | ERROR | S5 | Неисправимая ошибка, ручное вмешательство |
| 16 | KILLED | S5 | Kill-switch, всё остановлено |

## Персистентность

Схема `execution_state` (PK = `symbol`), `migrations/0003_execution_state.sql` + S6 v2 (`0004`) + **S7 halt persistence (`0005`)**. Decimal хранится как TEXT. Coordinator пишет на каждом конце перехода (exchange побеждает per ADR 0019).

### S7 столбцы (ADR 0021 sub-decision 5)

| Column | Тип | Назначение |
|---|---|---|
| `halt_reason` | TEXT NULL | Primary wins — первая non-null "липнет". Не перезаписывается до `MANUAL_RESET` |
| `last_exit_reason` | TEXT NULL | Код причины последнего exit (audit trail) |
| `last_reconcile_at` | TEXT (ISO-8601 UTC) | Время последней успешной сверки |
| `bootstrap_at` | TEXT (ISO-8601 UTC) | Время `Coordinator.bootstrap()` (якорь идемпотентности) |

### S7 таблица аудита (ADR 0021 sub-decision 9)
...
Write-ahead паттерн: запись в `halt_log` **до** обновления `execution_state.halt_reason`. Гарантирует chronological trail даже при crash между write+state-update.
...
| `RECONCILE_ENTRY_FILLED` | **S7** | Вердикт reconciler `HEAL_ENTRY_FILLED` (entry заполнен офлайн; heal local) |
| `RECONCILE_EXITED` | **S7** | Вердикт reconciler `EXITED` (exit заполнен офлайн; перейти в FLAT с `EXIT_RECONCILE_DETECTED`) |
...
### S7 заметка о дедубликации (ADR 0021 follow-up)
...
**Update Sprint 7:** `halt_reason` теперь **persisted** в `execution_state.halt_reason` (ADR 0021 sub-decision 5) + audit-trail в `halt_log` (sub-decision 9). Primary-wins семантика: первая non-null причина залипает до `MANUAL_RESET`. S6-only обработчики (без write через `state_repo.set_halt_reason()`) считаются legacy и подлежат миграции.

### S7 halts (ADR 0021)

Добавлены 2 новых halt-подсостояния (через `halt_reason`, не state-enum):
...
## Закрыто в S7 (ADR 0021)

Все три pre-S6 ограничения (startup reconcile / pending+WS_RECONNECT / fragile OCO main lookup) закрыты:

- **C1 закрыто (S6 + S7):** `Coordinator.bootstrap()` всегда вызывает `reconciler.reconcile()` для классификации; `_bootstrap_done` assert на всех external entry points.
- **C2 закрыто (S7):** `(ENTRY_PENDING|EXIT_PENDING, WS_RECONNECT) → RECONCILING` промотирует через 4-valued reconciler verdict (`AGREE` / `DIVERGENCE` / `HEAL_ENTRY_FILLED` / `EXITED`).
...
| # | Инвариант | Enforcement | Test |
|---|-----------|-------------|------|
| 1 | TRANSITIONS таблица-ориентированная — нет implicit if/else dispatch | `src/execution/state_machine.py::TRANSITIONS` dict | `tests/unit/test_execution_fsm.py::test_transitions_count_exact` |
...
| 3 | `WS_RECONNECT` валиден для ровно 9 `_RECONCILABLE_STATES` | `src/execution/coordinator.py::_RECONCILABLE_STATES` frozenset | `tests/unit/test_coordinator_on_ws_reconnect.py::test_on_ws_reconnect_flat_state_is_noop` |
...
- `[[../../trading/concepts/reason-codes]]` — **50 кодов** (S7 baseline 42 + S36 `HALT_S36_*` +4 + S37 `HALT_UNKNOWN_SYMBOL` +1, per ADR 0055 + 0057).
