...
# Current State (post-S44, 2026-05-10) — WFA retrofit (research presets acceptance gate restored, all 11 WFA_FAIL honest verdict, tag v0.1.0-alpha.44)
...
| Reason codes | **56** | `src/risk/reason_codes.py` `ReasonCode` enum | S40 (+3 ENTRY_LONG_ATR_BREAKOUT / EXIT_FLAT_ATR_REVERSE / EXIT_FLAT_ATR_STOP_AB per ADR 0060) |
...
| ADRs | **64** | `wiki/project/decisions/*.md` (0001-0064) | S44 (ADR 0064 WFA retrofit) |
| Sprint pages | **48** | `wiki/project/sprints/sprint-*.md` (sprint-01..sprint-44 + sprint-08a/b/c + sprint-32b/c/d/e, minus S24+S26) | S44 (sprint-44-wfa-retrofit) |
...
| **S44** | **0064** | **v0.1.0-alpha.44** | **2026-05-10** | **WFA retrofit** — research presets acceptance gate restored (T1-T6 + DSR + MC + N_trials). All 11 WFA_FAIL: 10 statistical + 1 data-limited (BTCUSDT 1D WFA_FAIL_DATA). Honest disclosure of OOS validation failure; pre-S44 RAW verdicts (+819% headline) hid this. ~970 unit / ~58 integration / mypy 0. Canonical 16/30/74/56 unchanged. |
| **S43** | **0063** | **v0.1.0-alpha.43** | **2026-05-10** | **UI polish** — preset rename к semantic RU + optgroup grouping (Тренд-следование / Возврат к среднему / Прорывы) + per-strategy description block + equity curve chart (uPlot v1.6.31 vendored). ~955 unit / ~54 integration / mypy 0. Canonical 16/30/74/56 unchanged. |
| **S42** | **0062** | **v0.1.0-alpha.42** | **2026-05-10** | **atr_breakout production hardening (kit retrofit)** — envelope helper `build_research_runner_envelope()`, preset consolidation 10→1, `/api/strategy/{id}/info` endpoint, JS defensive guards, RAW_FULL_PERIOD honest label. 946 unit / 52 integration / mypy 0. Supersedes ADR 0060+0061. |
| **S41** | **0061** | **v0.1.0-alpha.41** | **2026-05-10** | **ATR breakout multi-combo dashboard presets** — 9 новых (symbol, interval) комбо из endless autoresearch. Независимые locked params per combo (anti-snooping, ADR 0061). +9 STRATEGY_PRESETS / обобщён runner (params kwarg + PARQUET_BY_COMBO + per-interval BARS_PER_YEAR) / 20 новых integration tests. Все комбо (Bybit 3.3y 2023→2026): SOLUSDT 4H +264% / ETHUSDT 1H +182% / BTCUSDT 15M +107% / BTCUSDT 1H +146% / SOLUSDT 1H +214% / ETHUSDT 4H +152% / SOLUSDT 15M +151% / BTCUSDT 1D +168% / ETHUSDT 15M +36%. 934 unit tests pass / mypy 0 / 16/30/74/**56**. |
| **S40** | **0060** | **v0.1.0-alpha.40** | **2026-05-10** | **atr_breakout production integration** — autoresearch iter1 winner LOCKED (BTCUSDT 4H, atr_period=9, atr_breakout_mult=2.5, atr_stop_period=21, atr_stop_mult=1.5). +3 ReasonCodes / ATRBreakoutStrategy / production runner (Binance 8.7y) / dashboard ENFORCE preset. 8.7y baseline: +819.81% / Sharpe 1.11 / 69 trades / 5/5 sub-periods positive (первый 5/5 в истории). 7 tasks TDD / 8 integration HARD-GATE tests. ~934→~952 tests / mypy 0 / 16/30/74/**56**. |
| **S39** | **0059** | **v0.1.0-alpha.39** | **2026-05-09** | **volume_breakout production integration + tech debt** — autoresearch iter 10 winner sweep#1644 LOCKED (BTCUSDT 4H, breakout_period=20, volume_mult=1.5). +3 ReasonCodes / VolumeBreakoutStrategy / dashboard ENFORCE preset / T5b BLOCKER fix (production runner ±0.5% baseline) / H1 rate-limit backoff / H2 WS reconnect / Item#10 boundary tests / Item#7 shim removal (22 callers) / M3+M4 bybit-api hardening. 8mo held-out PRIMARY: Sharpe +9.96 / PnL +20.42% / n=17. 905→915+ tests / mypy 0 / 16/30/74/**53**. Gate 2 paper-trade PENDING. |
| **S38** | **0058** | **v0.1.0-alpha.38** | **2026-04-27** | **δ Parallel Hardening** — ROUND 6 consilium UNANIMOUS Path A (δ activate now + S38 parallel). 7 tasks: ADR 0058 + ADR 0056 amendment 2 LOCKED / F2 quant pnl_pct fix (compute_live_sharpe + MC) / bybit-api-reviewer first invocation 6-axis (0 BLOCKER, 3 HIGH, 4 MED, 3 LOW, 10 VERIFIED) / Item #7 RiskSharedDeps Demeter refactor (DI ONLY) / ADR 0057 amendment months_since truncation / playbook 5 NEW gates + UNDERPOWERED + halt-triggered + T3 H3 accountType / sync. δ TESTNET activate operator-side parallel. 8 NEW tests / pytest 905+33 / mypy 0. 1 ADR + 2 amendments + 1 sprint + 1 query |
| **S37** | **0057** | **v0.1.0-alpha.37** | **2026-04-27** | **Carry-overs hardening** — ROUND 5 consilium binding. 8 tasks: ADR 0057+0056 amendment LOCKED / symbol whitelist + fail-closed + HALT_UNKNOWN_SYMBOL (49→50) / activation_ts HMAC / clock injection / coord.symbol property / DSR boundary + S22 6.17→2.96 / playbook / sync. δ TESTNET production-ready. 26 NEW tests / pytest 897+33 / mypy 0. 1 ADR + 1 amendment + 1 sprint + 1 component |
| **S36** | **0055+0056** | **v0.1.0-alpha.36** | **2026-04-27** | **δ TESTNET activation** — ROUND 4 consilium binding. 8 tasks: ADR 0055+0056 LOCKED (anti-snooping) + B1 CRITICAL fix (MEAN_REVERSION_S17_RELAXED_PARAMS wired к live path) + 4 state-source methods + HaltGate wire-up в _tick (7 integration tests) + ReasonCode +4 HALT_S36_* (45→49) + DSR sigma_SR amendment (sigma_SR hierarchy + n_trades thresholds + variable rename + fallback REMOVED) + live trade reporter (live Sharpe + calibration + MC gating) + wiki sync. δ infrastructure WIRED LIVE. 63 NEW tests / pytest 871+33 / mypy 0. 2 ADRs + 1 sprint + 2 components |
