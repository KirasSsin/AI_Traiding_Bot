...
| Component pages | **40** | `wiki/project/components/*.md` (incl. README.md cluster index) | S32b +2 (no change S32c) |
| ADRs | **47** | `wiki/project/decisions/*.md` (0001-0047) | S32c (ADR 0047 — Kit Phase 2) |
| Sprint pages | **34** | `wiki/project/sprints/sprint-*.md` (sprint-01..sprint-32 + sprint-08a/b/c + sprint-32b/c, minus S24+S26) | S32c (sprint-32c-kit-phase-2-improvements) |
| Reviewer agents | **10** | `~/.claude/agents/` (out-of-repo) | S32b +1 (no change S32c) |
| Active push hooks | **7** | `~/.claude/hooks/` (PreToolUse Bash) | S32b +sprint-state-freshness-check.sh (no change S32c) |
| MCP servers | **8** | settings.json + .mcp.json | S32c +fetch (.mcp.json) |
| CI gates | **YES** | `.github/workflows/ci.yml` (push к main + PR) | S32b NEW (no change S32c) |
| Pre-commit gates | **YES** | `.pre-commit-config.yaml` (ruff + mypy + yamllint) | S32b upgraded (no change S32c) |
| Skills × Phase mapped | **36** | sprint-flow-ru.md Skills × Phase integration map | S32c +4 AS (api-design/browser-test/perf-opt/idea-refine extension) |
