...
- **T9 formal WFA:** WFA_FAIL. **Primary failure = T5 n_eff < 50** (gate-blocking, DSR-independent, bug-free — quant PHASE 6). Verdict стоит на T5 один; никакой DSR не спасёт <50 OOS trades. MC p=0.0005 PASS.
- **DSR=0.0 — UNINFORMATIVE (artifactual, НЕ доказательство):** quant PHASE 6 — cross-trial pool содержит 8 S44 ATR-breakout Sharpes (BTC/ETH/SOL, −89…+0.37), sigma_sr≈34 → Bailey sharpe_star≈54 = unbeatable bar для ЛЮБОЙ стратегии. Bailey eq.12 i.i.d.-trials предпосылка нарушена смешением strategy classes + symbols. DSR=0.0 механический, не сигнал про Supertrend. Per-strategy-class pool scoping = open ADR (carry S51).
- **PHASE 6 BLOCKER fixed (`48dac93`):** backtest fill look-ahead (flip@close[i] → open[i] вместо open[i+1], 3 пути). Post-fix held-out Sharpe **8.08 → −4.23**, full-range WFA OOS mean **−9.38** (inflation убрана, сигнал отрицательный OOS — был look-ahead-inflated 3.6-8.7 по всей grid). Verdict stays WFA_FAIL, harder. cross_trial entry corrected (BTCUSDT_7_3.0, oos −9.38; tag fix `4a6282c`).
