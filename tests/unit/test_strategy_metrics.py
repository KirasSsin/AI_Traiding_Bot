...
def _make_trade(
    *,
    ...
    entry_price_decimal: Decimal = Decimal("50000"),
) -> TradeRecord:
...
def test_compute_metrics_t1_sharpe_winners_positive() -> None:
    # S49 BATCH 8: use a varied winning series. All-identical pnl → std=0 →
    # t1_sharpe is now NaN (correct, per the inf-guard fix). A positive Sharpe
    # is only meaningful when the series has non-zero variance.
    trades = [
        _make_trade(pnl_quote=Decimal("2.0") if i % 2 == 0 else Decimal("1.0"), hours_offset=i)
        for i in range(100)
    ]
...
def test_compute_metrics_t3_initial_capital_parameterizable() -> None:
    ...
    metrics_50k = compute_t1_t6_metrics(
        trades=trades, fold_oos_is_sharpe=[1.0], initial_capital=50000.0
    )
...
def test_compute_metrics_t3_first_loss_drawdown_from_initial_capital() -> None:
    ...
    trades = [
        _make_trade(
            pnl_quote=Decimal("-500"),
            hours_offset=0,
            qty_decimal=Decimal("0.1"),
            entry_price_decimal=Decimal("50000"),
        )
    ]
    metrics = compute_t1_t6_metrics(
        trades=trades, fold_oos_is_sharpe=[1.0], initial_capital=10000.0
    )
...
# ---------------------------------------------------------------------------
# S49 BATCH 8 NEW-BUG: zero-std-dev Sharpe/Sortino/t-stat guard → NaN not inf
# (found by BATCH 7 property test). Degenerate all-identical pnl series → std≈0.
# ---------------------------------------------------------------------------


def test_t1_sharpe_nan_not_inf_on_zero_std() -> None:
    """All-identical-pnl series → std=0 → t1_sharpe is NaN, NOT ±inf."""
    import math

    # All trades identical positive pnl → std(ddof=1)=0.
    trades = [_make_trade(pnl_quote=Decimal("1.0"), hours_offset=i) for i in range(50)]
    metrics = compute_t1_t6_metrics(trades=trades, fold_oos_is_sharpe=[1.0])
    assert math.isnan(metrics["t1_sharpe_oos"])
    assert not math.isinf(metrics["t1_sharpe_oos"])
    # t-stat shares the same denominator → also NaN, not inf.
    assert math.isnan(metrics["t5_t_stat"])
    assert not math.isinf(metrics["t5_t_stat"])


def test_t2_sortino_nan_not_inf_on_all_winners() -> None:
    """All winners → downside_dev=0 → t2_sortino is NaN, NOT ±inf."""
    import math

    trades = [_make_trade(pnl_quote=Decimal("1.0"), hours_offset=i) for i in range(50)]
    metrics = compute_t1_t6_metrics(trades=trades, fold_oos_is_sharpe=[1.0])
    assert math.isnan(metrics["t2_sortino_oos"])
    assert not math.isinf(metrics["t2_sortino_oos"])


def test_t1_sharpe_finite_on_normal_series() -> None:
    """Normal mixed-pnl series → finite (non-NaN, non-inf) Sharpe — unchanged behavior."""
    import math

    trades = [_make_trade(pnl_quote=Decimal("2.0"), hours_offset=i) for i in range(60)] + [
        _make_trade(pnl_quote=Decimal("-1.0"), hours_offset=60 + i) for i in range(40)
    ]
    metrics = compute_t1_t6_metrics(trades=trades, fold_oos_is_sharpe=[1.0])
    assert math.isfinite(metrics["t1_sharpe_oos"])
    assert math.isfinite(metrics["t2_sortino_oos"])
    assert math.isfinite(metrics["t5_t_stat"])
