...
def test_flatten_from_long_open_both_fail_halts_cleanly(coordinator_armed_harness):
    """S55 PHASE6 TL-NEW-01: flatten() from LONG_OPEN with BOTH sells failing must
    reach HALTED with halt_reason=HALT_FLATTEN_FAILED via a CLEAN FLATTEN_FAILED
    transition — no IllegalTransitionError, no spurious second HALT_RUNTIME_CRASH
    halt_log entry.

    TL-01 added LONG_OPEN to _FLATTENABLE_STATES, but (LONG_OPEN, FLATTEN_FAILED)
    was missing from TRANSITIONS → KeyError → IllegalTransitionError → crash-halt
    cascade appended a spurious HALT_RUNTIME_CRASH audit row.
    """
    import dataclasses

    h = coordinator_armed_harness
    # Re-pin the persisted row to LONG_OPEN (fixture seeds OCO_ARMED).
    cur = h.repo.get("BTCUSDT")
    h.repo.upsert(dataclasses.replace(cur, state=ExecutionState.LONG_OPEN))
    ...
    row = h.repo.get("BTCUSDT")
    assert row.state == ExecutionState.HALTED
    assert row.halt_reason == ReasonCode.HALT_FLATTEN_FAILED.value

    # Audit trail: exactly ONE halt_log row, and it is the flatten-failed halt —
    # no spurious HALT_RUNTIME_CRASH entry from an IllegalTransition crash path.
    reasons = [
        r[0]
        for r in h.repo._conn.execute(
            "SELECT reason FROM halt_log WHERE symbol = ?", ("BTCUSDT",)
        ).fetchall()
    ]
    assert reasons == [ReasonCode.HALT_FLATTEN_FAILED.value]


def test_flatten_from_oco_arming_both_fail_halts_cleanly(coordinator_armed_harness):
    """S55 PHASE6 TL-NEW-01: same as above but from OCO_ARMING (also made
    flattenable by TL-01). Clean HALTED, single halt_log entry."""
    import dataclasses

    h = coordinator_armed_harness
    cur = h.repo.get("BTCUSDT")
    h.repo.upsert(dataclasses.replace(cur, state=ExecutionState.OCO_ARMING))

    h.adapter.fail_market_sell_count = 2
    h.coordinator.flatten(reason=ReasonCode.HALT_RECONCILE_DIVERGENCE)
