...
def test_cmd_wfa_invokes_walk_forward_runner_returns_zero_on_pass() -> None:
    ...
    with (
        patch("src.__main__.WalkForwardRunner") as mock_runner_class,
...
def test_load_ohlcv_raises_helpful_error_when_parquet_missing():
    ...
    with (
        patch("src.__main__.load_market_data", side_effect=FileNotFoundError("no such file")),
...
def test_load_ohlcv_accepts_valid_symbol(symbol):
    ...
    with patch("src.__main__.load_market_data", return_value=fake_df):
