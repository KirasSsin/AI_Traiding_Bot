...
def _seed(
    ...
    *,
    ...
    tp_oid: str | None = "TP-OID",
    sl_oid: str | None = "SL-OID",
) -> None:
    ...
    repo.upsert(
        ExecutionStateRow(
            ...
            last_attempt_num=1,
            updated_at=now,
            # S55 TL-01: planned exit prices — start_bracket persists these so the
            # entry-Filled handler can arm the OCO legs after the fill.
            bracket_tp_price=Decimal("70000"),
            bracket_sl_trigger_price=Decimal("60000"),
        )
    )
...
class _StubAdapter:
    def __init__(self) -> None:
        ...
        self.stop_orders: list[str] = []
        from src.marketdata.filters import BybitFilters

        self._filters = BybitFilters(
            symbol="BTCUSDT",
    ...
    def place_limit_order(self, *, symbol, side, qty, price, order_link_id):
        ...
        self.limit_orders.append(order_link_id)
    ...
    def place_stop_market_order(self, *, symbol, side, qty, trigger_price, order_link_id):
        ...
        self.stop_orders.append(order_link_id)
        return OrderAck(order_id=f"EX-{order_link_id}", order_link_id=order_link_id)
...
def test_entry_filled_arms_oco_bracket(tmp_path):
    """S55 TL-01: entry Filled drives ENTRY_PENDING → LONG_OPEN → arm → OCO_ARMED.

    Pre-S55 this only reached LONG_OPEN and stopped (the unbounded-loss defect:
    arm_oco was never called). The entry-Filled handler now arms the OCO legs.
    """
    ...
    assert len(adapter.limit_orders) == 1  # TP leg placed on fill
    assert len(adapter.stop_orders) == 1  # SL leg placed on fill
