...
def test_load_ohlcv_passes_when_dropna_yields_above_90pct() -> None:
    ...
    with patch("src.backtest.data_loading.load_market_data", return_value=df):
...
def test_load_ohlcv_aborts_when_dropna_yields_below_90pct() -> None:
    ...
    with (
        patch("src.backtest.data_loading.load_market_data", return_value=df),
        pytest.raises(ValueError, match="NaN.*pre-flight.*90%"),
    ):
