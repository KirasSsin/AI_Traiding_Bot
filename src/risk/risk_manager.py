...
class RiskManager:
    ...
    def update_capital(self, new_capital: float):
        ...
        if drawdown_pct >= self.max_daily_drawdown_pct:
            logger.critical(
                f"KILL-SWITCH ENGAGED! Drawdown {drawdown_pct * 100:.2f}% exceeds {self.max_daily_drawdown_pct * 100:.2f}% limit."
            )
    ...
    def calculate_position_size(self, signal: Signal) -> float:
        ...
        if len(self.recent_trade_returns) >= 10:
            kelly_pct = MathEngine.fractional_kelly(
                self.win_rate, self.win_loss_ratio, fraction=0.5
            )
    ...
    def evaluate_signal(self, signal: Signal) -> bool:
        ...
        if notional_value > self.current_capital:
            logger.warning(
                f"Signal rejected: Notional value ({notional_value}) exceeds current capital ({self.current_capital})."
            )
            return False

        logger.info(
            f"Signal approved: {signal.direction} {signal.symbol}. Calculated size: {position_size:.6f}"
        )
        return True
